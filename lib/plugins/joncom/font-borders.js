    ...
    ig.Font.inject({
        ...
        onload: function(event) {
            ...
            if(this.borderColor && this.borderSize >= 1) {
                ...
                // Create font layer.
                var fontCanvas = this.data;
                var fontPath = this.getBasePath() + '?' +
                               'layer=font' +
                               '&size=' + this.borderSize;
                if(this.fontColor) {
                    fontPath += '&color=' + this.fontColor;
                }
                this.fontLayer = new ig.CanvasImage(fontPath, fontCanvas);
                // Create border layer.
                var borderCanvas = this._createBorderCanvas(fontCanvas);
                var borderPath = this.getBasePath() + '?' +
                                 'layer=border' +
                                 '&size=' + this.borderSize +
                                 '&color=' + this.borderColor;
                this.borderLayer = new ig.CanvasImage(borderPath, borderCanvas);
                // Create line layer.
                var lineCanvas = this._createLineCanvas();
                var linePath = this.getBasePath() + '?' +
                               'layer=line' +
                               '&size=' + this.borderSize;
                this.lineLayer = new ig.CanvasImage(linePath, lineCanvas);

                // Merge font, border and line into a single canvas.
                var canvas = ig.$new('canvas');
                canvas.width = fontCanvas.width;
                canvas.height = fontCanvas.height;
