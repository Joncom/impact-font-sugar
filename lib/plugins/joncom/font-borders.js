    ...
    ig.Font.inject({
        ...
        onload: function(event) {
            ...
            if(this.borderColor) {
                ...
                var borderData = newContext.getImageData(0, 0, canvas.width, canvas.height);
                ...
                for (var c = 0; c < this.widthMap.length; c++) {
                    ...
                    for (var i = first; i < last; i++) {
                        borderData.data[((borderData.width * (canvas.height - 1)) + i) * 4] = 255; // red
                        borderData.data[((borderData.width * (canvas.height - 1)) + i) * 4 + 1] = 0; // green
                        borderData.data[((borderData.width * (canvas.height - 1)) + i) * 4 + 2] = 0; // blue
                        borderData.data[((borderData.width * (canvas.height - 1)) + i) * 4 + 3] = 255; // alpha
                    }
                }

                // Merge border data with font fill data.
                var fontFillImage = new Image();
                fontFillImage.src = canvas.toDataURL("image/png");
                fontFillImage.onload = function() {
                    newContext.putImageData(borderData, 0, 0);
                    newContext.drawImage(fontFillImage, 0, 0);
                };
