...
ig.module('plugins.joncom.font-borders')
    ...
    ig.Font.inject({
        ...
        onload: function(event) {
            ...
            if(this.borderColor) {
                ...
                var fontCanvas = ig.$new('canvas');
                var borderCanvas = ig.$new('canvas');

                canvas.width = this._getNewFontWidth();
                fontCanvas.width = canvas.width;
                borderCanvas.width = canvas.width;

                canvas.height = this._getNewFontHeight();
                fontCanvas.height = canvas.height;
                ...
                var borderData = borderContext.getImageData(0, 0, canvas.width, canvas.height);
                ...
                for (var c = 0; c < this.widthMap.length; c++) {
                    ...
                    // Check and fix for a DOM exception error caused by some fonts
                    // having extra space to the right of the last character.
                    if (x >= oldContext.canvas.width) {
                        continue;
                    }
                    ...
                    // Fill in the font character.
                    fontContext.putImageData(charData, x + offsetX, y + offsetY);
                        ...
                        for (var py in pixels[px]) {
                            ...
                            for (var bx in borderPixels) {
                                ...
                                for (var by in borderPixels[bx]) {
                                    ...
                                    borderData.data[((borderData.width * (y + by + offsetY)) + x + bx + offsetX) * 4] = borderRGB.r; // red
                                    borderData.data[((borderData.width * (y + by + offsetY)) + x + bx + offsetX) * 4 + 1] = borderRGB.g; // green
                                    borderData.data[((borderData.width * (y + by + offsetY)) + x + bx + offsetX) * 4 + 2] = borderRGB.b; // blue
                    ...
                    for (var i = first; i < last; i++) {
                        borderData.data[((borderData.width * (borderCanvas.height - 1)) + i) * 4] = 255; // red
                        borderData.data[((borderData.width * (borderCanvas.height - 1)) + i) * 4 + 1] = 0; // green
                        borderData.data[((borderData.width * (borderCanvas.height - 1)) + i) * 4 + 2] = 0; // blue
                        borderData.data[((borderData.width * (borderCanvas.height - 1)) + i) * 4 + 3] = 255; // alpha
                    }
                }

                borderContext.putImageData(borderData, 0, 0);

                // Merge border data with font data.
                var fontImage = new Image();
                fontImage.src = fontCanvas.toDataURL("image/png");
                fontImage.onload = function() {
                    var borderImage = new Image();
                    borderImage.src = borderCanvas.toDataURL("image/png");
                    borderImage.onload = function() {
                        newContext.drawImage(fontImage, 0, 0);
                        newContext.drawImage(borderImage, 0, 0);
                    };
                };
        ...
        /*
        // Test case
        var test = ig.game.font8._createBorderCanvas(ig.game.font8.fontCanvas, {});
        testImage = new Image();
        testImage.src = test.toDataURL("image/png");
        testImage.onload = function() {
          console.log(testImage.src);
        }
        */
        _createBorderCanvas: function(data, color) {
            var canvas = ig.$new('canvas');
        ...
        _getNewPath: function(path) {
            return path +
                   ( this.borderColor ? this._makeHexSafe(this.borderColor) : '' ) +
                   '#' +
                   ( this.fontColor ? this._makeHexSafe(this.fontColor) : '' ) +
                   '#' +
                   this.borderSize;
        },
