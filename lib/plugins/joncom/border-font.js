...
.defines(function() {
    ...
    ig.BorderFont = ig.Font.extend({
        ...
        onload: function(event) {
            ...
            // Colors
            var fontRGB = this._getRGBFromHex(this.fontColor);
            ...
            for(var c=0; c<this.widthMap.length; c++) {
                ...
                var charData = oldCtx.getImageData(x, y, width, height);
                ...
                for (var px in pixels) {
                    ...
                    for(var py in pixels[px]) {
                        ...
                        // Fill character body.
                        borderData.data[((borderData.width * (y+py+offsetY)) + x+px+offsetX) * 4] = fontRGB.r; // red
                        borderData.data[((borderData.width * (y+py+offsetY)) + x+px+offsetX) * 4 + 1] = fontRGB.g; // green
                        borderData.data[((borderData.width * (y+py+offsetY)) + x+px+offsetX) * 4 + 2] = fontRGB.b; // blue
                        borderData.data[((borderData.width * (y+py+offsetY)) + x+px+offsetX) * 4 + 3] = 255; // alpha
                    }
                }
            }
