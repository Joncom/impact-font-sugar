...
.defines(function() {
    ...
    ig.BorderFont = ig.Font.extend({
                    ...
                    for(var py in pixels[px]) {
                        ...
                        for (var bx in borderPixels) {
                            ...
                            for(var by in borderPixels[bx]) {
                                by = parseInt(by);
                                borderData.data[((borderData.width * (by+y)) + bx+x) * 4] = 255; // red
                                borderData.data[((borderData.width * (by+y)) + bx+x) * 4 + 1] = 255; // green
                                borderData.data[((borderData.width * (by+y)) + bx+x) * 4 + 2] = 255; // green
                                borderData.data[((borderData.width * (by+y)) + bx+x) * 4 + 3] = 255; // alpha
                            }
                        }
                    }
                }
        ...
        _getBorderPixels: function(x, y) {
            ...
            pixels[x] = {};
            pixels[x+1] = {};
            pixels[x-1][y-1] = true; // up & left
            pixels[x-1][y]   = true; // left
            pixels[x-1][y+1] = true; // down & left
            pixels[x][y-1]   = true; // up
            pixels[x][y+1]   = true; // down
            pixels[x+1][y-1] = true; // up & right
            pixels[x+1][y]   = true; // right
            pixels[x+1][y+1] = true; // down & right
            return pixels;
        },
        ...
        _getNewFontHeight: function() {
            return (this.height - 2) + this.borderSize * 2; // -2 because bottom lines contain no font.
        },
