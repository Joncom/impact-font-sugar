    ...
    ig.BorderFont = ig.Font.extend({
        ...
        onload: function(event) {
                ...
                for (var px in pixels) {
                    for(var py in pixels[px]) {
                        for(var up = 0; up < this.borderSize; up++) {
                            borderData.data[((borderData.width * py) + px) * 4] = 255; // red
                            borderData.data[((borderData.width * py) + px) * 4 + 1] = 0; // green
                            borderData.data[((borderData.width * py) + px) * 4 + 2] = 0; // green
                            borderData.data[((borderData.width * py) + px) * 4 + 3] = 255; // alpha
                        }
                    }
                }
            }
            ...
            for(var y = 0; y < canvas.height; y++) {
                // loop through each columngit pu
