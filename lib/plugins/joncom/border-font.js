    ...
    ig.BorderFont = ig.Font.extend({
        ...
        onload: function(event) {
            ...
            for(var c=0; c<this.widthMap.length; c++) {
                ...
                var pixels = this._getNonAlphaPixels(data);
                // Loop through non-alpha pixels.
                for (var px in pixels) {
                    for(var py in pixels[px]) {
                        //for(var up = 0; up < this.borderSize; up++) {
                            console.log(y);
                            borderData.data[((borderData.width * (parseInt(py)+y)) + parseInt(px)+x) * 4] = 255; // red
                            borderData.data[((borderData.width * (parseInt(py)+y)) + parseInt(px)+x) * 4 + 1] = 0; // green
                            borderData.data[((borderData.width * (parseInt(py)+y)) + parseInt(px)+x) * 4 + 2] = 0; // green
                            borderData.data[((borderData.width * (parseInt(py)+y)) + parseInt(px)+x) * 4 + 3] = 255; // alpha
                        //}
                    }
                }
            }
            ...
            for(var y = 0; y < canvas.height; y++) {
                ...
                borderData.data[((canvas.width * y) + x) * 4] = 255;
                borderData.data[((canvas.width * y) + x) * 4 + 1] = 0;
                borderData.data[((canvas.width * y) + x) * 4 + 2] = 0;
                borderData.data[((canvas.width * y) + x) * 4 + 3] = 255;
                }
            }
