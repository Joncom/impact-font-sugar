...
.defines(function() {
    ...
    ig.BorderFont = ig.Font.extend({
        ...
        init: function(path, settings) {
            ...
            // Add # is missing from colors.
            if(this.borderColor.charAt(0) !== '#') this.borderColor = '#' + this.borderColor;
            if(this.fontColor && this.fontColor.charAt(0) !== '#') this.fontColor = '#' + this.fontColor;

            var newPath = path + this.borderColor + ( this.fontColor ? this.fontColor : '' );
            this.parent(newPath);
        },
